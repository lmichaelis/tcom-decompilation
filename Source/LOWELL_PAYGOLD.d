var int LOWELL_PAYGOLD;
var int LOWELL_CLEARSWAMP;
...
func int DIA_LOWELL_WORK_CONDITION() {
    if ((NPC_KNOWSINFO(OTHER, 85431)) && ((LOG_GETSTATUS(MIS_Q401)) == (LOG_RUNNING))) {
        if ((Q401_FABIOANDGANGGONE) == (0)) {
            return TRUE;
        };
    };
...
func void DIA_LOWELL_SWAMP() {
    ...
    if ((Q401_SWAMPMONSTERS) == (FALSE)) {
        ...
        INFO_CLEARCHOICES(85435);
        INFO_CLEARCHOICES(85442);
        AI_STOPPROCESSINFOS(SELF);
    };
...
func void DIA_LOWELL_WORK_INFO() {
    ...
    INFO_CLEARCHOICES(85435);
    INFO_ADDCHOICE(85435, "In that case, here. (250 GP)", 85440);
    INFO_ADDCHOICE(85435, "I'll come back later.", 85441);
}

func void DIA_LOWELL_WORK_GOLD() {
    if ((NPC_HASITEMS(OTHER, 34203)) >= (Q401_CUTTERSPRICE)) {
        NPC_REMOVEINVITEMS(SELF, 34203, Q401_CUTTERSPRICE);
        AI_OUTPUT(OTHER, SELF, "DIA_Lowell_Work_Gold_15_01");
        B_GIVEINVITEMS(OTHER, SELF, 34203, Q401_CUTTERSPRICE);
        DIA_LOWELL_SWAMP();
    };
    LOWELL_PAYGOLD = TRUE;
...
func int DIA_LOWELL_YOURGOLD_CONDITION() {
    if (((NPC_KNOWSINFO(OTHER, 85435)) && ((LOG_GETSTATUS(MIS_Q401)) == (LOG_RUNNING))) && ((LOWELL_PAYGOLD) == (TRUE))) {
        if ((Q401_FABIOANDGANGGONE) == (0)) {
            return TRUE;
        };
    };
...
func int DIA_LOWELL_Q504_FEEL_CONDITION() {
    if ((((Q504_TAKEWORKERS) == (2)) && ((NPC_GETDISTTOWP(SELF, "PART5_WORKERSCAMP_SAW")) <= (2000))) && ((Q504_LOWELLVOLFZACKE) == (1))) {
        return TRUE;
    };
    return 0 /* !broken stack! */;
}
...
func int DIA_LOWELL_AMBIENT_CONDITION() {
    if ((NPC_KNOWSINFO(OTHER, 85451)) || (NPC_KNOWSINFO(OTHER, 85431))) {
        if ((NPC_GETDISTTOWP(SELF, "PART5_WORKERSCAMP_SAW")) > (2000)) {
            return TRUE;
        };
    };
    return 0 /* !broken stack! */;
...
func void DIA_LOWELL_AMBIENT_INFO() {
    ...
    if ((!(NPC_KNOWSINFO(OTHER, 85431))) && ((Q401_LOWELL_AMBIENT) == (FALSE))) {
...
func void DIA_LOWELL_PICKPOCKET_INFO() {
    INFO_CLEARCHOICES(85457);
    INFO_ADDCHOICE(85457, DIALOG_BACK, 85461);
    INFO_ADDCHOICE(85457, DIALOG_PICKPOCKET, 85460);
}

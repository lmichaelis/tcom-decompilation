...
func void DIA_HELMUD_HELLO_INFO() {
    ...
    if ((((SQ101_SEARCHINGFORNIRKO) == (TRUE)) && ((LOG_GETSTATUS(MIS_SQ101)) == (LOG_RUNNING))) && ((SQ101_TALKEDWITHHELMUDABOUTNIRKO) == (FALSE))) {
        INFO_CLEARCHOICES(85465);
        INFO_ADDCHOICE(85465, "Actually, I'm looking for a man named Elco.", 85469);
    };
}
...
func void DIA_HELMUD_SQ117_START_INFO() {
    ...
    INFO_CLEARCHOICES(85473);
    INFO_ADDCHOICE(85473, "I'll take care of them. Where are they?", 85478);
    INFO_ADDCHOICE(85473, "You're on your own.", 85482);
    INFO_ADDCHOICE(85473, "It depends on what I get in return.", 85479);
    INFO_ADDCHOICE(85473, "You should ask hunters for help.", 85480);
}

var int HELMUD_SQ117_TAKEQUEST;
...
func void DIA_HELMUD_SQ117_START_HUNTERS() {
    ...
    AI_RESETFACEANI(SELF);
    if (NPC_KNOWSINFO(OTHER, 85371)) {
        INFO_ADDCHOICE(85473, "The people of Wolf's Den didn't try to help you?", 85481);
    };
}
...
func int DIA_HELMUD_SQ117_TAKEQUEST_CONDITION() {
    if ((NPC_KNOWSINFO(OTHER, 85473)) && ((HELMUD_SQ117_TAKEQUEST) == (1))) {
        return TRUE;
    };
    return 0 /* !broken stack! */;
}
...
func void DIA_HELMUD_SQ117_FINISH_INFO() {
    ...
    if ((SQ117_ALLSNAPPERSDEAD) == (1)) {
        ...
        CREATEINVITEMS(SELF, 35895, 1);
        B_GIVEINVITEMS(SELF, OTHER, 35895, 1);
        CREATEINVITEMS(SELF, 34203, SQ117_FINISH);
    };
...
func void DIA_HELMUD_Q505_HELP_INFO() {
    ...
    CREATEINVITEMS(SELF, 37578, 1);
    B_GIVEINVITEMS(SELF, OTHER, 37578, 1);
    AI_RESETFACEANI(SELF);
    AI_RESETFACEANI(OTHER);
}
...
var int HELMUD_COUNTBOUGHTCOAL;
func void DIA_HELMUD_BUYCOAL_CHOICES() {
    var string BUYCOAL_5_SQ117;
    var string BUYCOAL_1_SQ117;
    var string BUYCOAL_5;
    var string BUYCOAL_1;
    var int COALCOST;
    COALCOST = VALUE_COAL;
    ...
    DUMMYFUNCWITHSTRINGPARAMETER(" SZ)");
    INFO_CLEARCHOICES(85498);
    INFO_ADDCHOICE(85498, DIALOG_BACK, 85511);
    if ((LOG_GETSTATUS(MIS_SQ117)) == (LOG_SUCCESS)) {
        INFO_ADDCHOICE(85498, BUYCOAL_5_SQ117, 85510);
        INFO_ADDCHOICE(85498, BUYCOAL_1_SQ117, 85509);
    };
    INFO_ADDCHOICE(85498, BUYCOAL_5, 85510);
    INFO_ADDCHOICE(85498, BUYCOAL_1, 85509);
}

func void DIA_HELMUD_BUYCOAL_INFO() {
    var int HELMUD_BUYCOALDISCOUNT;
...
func void DIA_HELMUD_BUYCOAL_1() {
    if ((LOG_GETSTATUS(MIS_SQ117)) == (LOG_SUCCESS)) {
        if ((NPC_HASITEMS(OTHER, 34203)) >= ((VALUE_COAL) / (2))) {
            HELMUD_COUNTBOUGHTCOAL = (HELMUD_COUNTBOUGHTCOAL) + (1);
            NPC_REMOVEINVITEMS(SELF, 34203, (VALUE_COAL) / (2));
            CREATEINVITEMS(SELF, 34228, 1);
        } else {
            B_SAY(SELF, OTHER, "$NOGOLD");
        };
    };
    if ((NPC_HASITEMS(OTHER, 34203)) >= (VALUE_COAL)) {
        HELMUD_COUNTBOUGHTCOAL = (HELMUD_COUNTBOUGHTCOAL) + (1);
        NPC_REMOVEINVITEMS(SELF, 34203, VALUE_COAL);
        CREATEINVITEMS(SELF, 34228, 1);
    };
    B_SAY(SELF, OTHER, "$NOGOLD");
    DIA_HELMUD_BUYCOAL_CHOICES();
...
func void DIA_HELMUD_BUYCOAL_5() {
    if ((LOG_GETSTATUS(MIS_SQ117)) == (LOG_SUCCESS)) {
        if ((NPC_HASITEMS(OTHER, 34203)) >= (((VALUE_COAL) * (5)) / (2))) {
            HELMUD_COUNTBOUGHTCOAL = (HELMUD_COUNTBOUGHTCOAL) + (5);
            NPC_REMOVEINVITEMS(SELF, 34203, ((VALUE_COAL) * (5)) / (2));
            CREATEINVITEMS(SELF, 34228, 5);
        } else {
            B_SAY(SELF, OTHER, "$NOGOLD");
        };
    };
    if ((NPC_HASITEMS(OTHER, 34203)) >= ((VALUE_COAL) * (5))) {
        HELMUD_COUNTBOUGHTCOAL = (HELMUD_COUNTBOUGHTCOAL) + (5);
        NPC_REMOVEINVITEMS(SELF, 34203, ((VALUE_COAL) * (5)) / (2));
        CREATEINVITEMS(SELF, 34228, 5);
    };
...
func void DIA_HELMUD_PICKPOCKET_INFO() {
    INFO_CLEARCHOICES(85512);
    INFO_ADDCHOICE(85512, DIALOG_BACK, 85516);
    INFO_ADDCHOICE(85512, DIALOG_PICKPOCKET, 85515);
}

func void DIA_HELMUD_PICKPOCKET_DOIT() {
    if ((NPC_GETTALENTSKILL(OTHER, NPC_TALENT_PICKPOCKET)) >= (2)) {
        CREATEINVITEMS(SELF, 34228, 4);
        B_GIVEINVITEMS(SELF, OTHER, 34228, 4);

...
func int DIA_BEN_KILLFOOL_CONDITION() {
    if ((NPC_ISINSTATE(SELF, 0xf09f)) && ((NPC_KNOWSINFO(OTHER, 0x1478e)) == (FALSE))) {
        return TRUE;
    };
...
func int DIA_BEN_HELLO_CONDITION() {
    if ((NPC_ISINSTATE(SELF, 0xf09f)) && (NPC_KNOWSINFO(OTHER, 0x1478e))) {
        return TRUE;
    };
    return 0 /* !broken stack! */;
}

var int BEN_HELLO_CORTEZ = 0;
var int BEN_HELLO_INFORMATION = 0;
var int BEN_HELLO_JOB = 0;
var int BEN_HELLO_SOMETHING = 0;
var int BEN_HELLO_GOAWAY = 0;
var int BEN_HELLO_GOLD = 0;
var int BEN_HELLO_JORN = 0;
var int BEN_HELLO_NOWAY = 0;
func void DIA_BEN_HELLO_CHOICES() {
    INFO_CLEARCHOICES(0x145b3);
    if ((BEN_HELLO_CORTEZ) == (FALSE)) {
        INFO_ADDCHOICE(0x145b3, "I thought Cortez was in charge here.", 0x145bf);
    };
    if ((BEN_HELLO_INFORMATION) == (FALSE)) {
        INFO_ADDCHOICE(0x145b3, "I just need one piece of information from you.", 0x145c0);
    };
    if (((OTHER.GUILD) == (GIL_MIL)) && ((BEN_HELLO_JOB) == (FALSE))) {
        INFO_ADDCHOICE(0x145b3, "I know you take all kinds of jobs, even kidnappings.", 0x145c1);
    };
    if ((BEN_HELLO_INFORMATION) == (TRUE)) {
        if ((BEN_HELLO_SOMETHING) == (FALSE)) {
            INFO_ADDCHOICE(0x145b3, "There must be something that convinces you to tell me something.", 0x145c3);
        } else if ((BEN_HELLO_GOLD) == (FALSE)) {
            INFO_ADDCHOICE(0x145b3, "I can pay you for any information.", 0x145c4);
        };
        if ((BEN_HELLO_JORN) == (FALSE)) {
            INFO_ADDCHOICE(0x145b3, "His name was Jorn. Does that name really mean nothing to you?", 0x145c5);
        };
        if ((BEN_HELLO_GOAWAY) == (FALSE)) {
            INFO_ADDCHOICE(0x145b3, "(Move along)", 0x145c7);
        };
    };
}
...
func void DIA_BEN_HELLO_CORTEZ() {
    ...
    if ((Q306_TOURNAMENTSTATUS) == (1)) {
        ...
        INFO_CLEARCHOICES(0x145b3);
        INFO_ADDCHOICE(0x145b3, "You didn't seem so threatening in the arena.", 0x145c2);
    };
...
func void DIA_BEN_HELLO_INFORMATION_SOMETHING_JORN() {
    ...
    INFO_CLEARCHOICES(0x145b3);
    INFO_ADDCHOICE(0x145b3, "I just want to know where he is.", 0x145c6);
}
...
func void DIA_BEN_HELLO_INFORMATION_GOAWAY() {
    ...
    if ((Q306_TOURNAMENTSTATUS) != (1)) {
        ...
        CREATEINVITEMS(SELF, 0x859b, Q306_BENREWARD);
    };
    AI_OUTPUT(SELF, OTHER, "DIA_Ben_HELLO_GoAway_03_05");
    AI_FUNCTION(SELF, 0x145c8);
}
...
func void BEN_PREPAREIVY() {
    ...
    TELEPORTNPCTOWP(0xe4af, "P17_HAVEN_BAR_OUT_03");
    AI_TELEPORTALWAYS(NONE_16_IVY, "P17_HAVEN_BAR_OUT_03", TRUE);
    PRINTD("Ivy przed karczm¹!");
}

func int DIA_BEN_POISON_CONDITION() {
    if (((NPC_ISINSTATE(SELF, 0xf09f)) && ((LOG_GETSTATUS(MIS_Q307)) == (LOG_RUNNING))) && ((Q307_WAITPOISONWAIT) == (TRUE))) {
        return TRUE;
    };
    return 0 /* !broken stack! */;
}
...
var int BEN_ANTIDOTECHOICE = 0;
func void BEN_KILLBEN_POISON() {
    NONE_7500_BEN.ATTRIBUTE[0] = 1;
    B_MAGICHURTNPC(HERO, NONE_7500_BEN, 0x1869f);
}

func void BEN_REMOVEMDS() {
    MDL_REMOVEOVERLAYMDS(NONE_7500_BEN, "HUMANS_HURT_BEN.MDS");
    NPC_CHANGEARMOR(NONE_7500_BEN, 0x8a38);
    NPC_REMOVEINVITEMS(NONE_7500_BEN, 0x8a37, NPC_HASITEMS(NONE_7500_BEN, 0x8a37));
}

func void DIA_BEN_POISON_END() {
    if ((BEN_Q307_DECISION) == (2)) {
        AI_FUNCTION(SELF, 0x145cf);
...
var int BEN_POISON_WHO = 0;
var int BEN_POISON_SECOND = 0;
var int BEN_POISON_TAKEOVER = 0;
var int BEN_POISON_SURE = 0;
var int BEN_POISON_SMUGGLERS = 0;
var int BEN_POISON_HOWMANY = 0;
var int BEN_POISON_RENEGADES = 0;
var int BEN_POISON_RING = 0;
var int BEN_POISON_WHATRING = 0;
var int BEN_POISON_KILL = 0;
var int BEN_POISON_KURT = 0;
var int BEN_POISON_SILBACH = 0;
var int BEN_POISON_DICKON = 0;
var int BEN_POISON_ESCAPE = 0;
func void DIA_BEN_POISON_CHOICES() {
    INFO_CLEARCHOICES(0x145c9);
    if ((BEN_POISON_WHO) == (FALSE)) {
        INFO_ADDCHOICE(0x145c9, "Who ordered you to kidnap him?", 0x145ed);
    };
    if ((BEN_POISON_SECOND) == (FALSE)) {
        if (((GQ001_INFOFROMDICKON) == (TRUE)) || ((GQ001_TALKEDWITHHUNTER) == (TRUE))) {
            INFO_ADDCHOICE(0x145c9, "Who was the other kidnapper?", 0x145ee);
        };
    };
    if ((BEN_POISON_TAKEOVER) == (FALSE)) {
        INFO_ADDCHOICE(0x145c9, "Who was supposed to take him over? You didn't see anyone?", 0x145ef);
    };
    if (((BEN_POISON_TAKEOVER) == (TRUE)) && ((BEN_POISON_SURE) == (FALSE))) {
        INFO_ADDCHOICE(0x145c9, "Are you sure you didn't see anyone?", 0x145f0);
    };
    if ((BEN_POISON_SMUGGLERS) == (FALSE)) {
        if ((LOG_GETSTATUS(MIS_Q206)) == (LOG_SUCCESS)) {
            INFO_ADDCHOICE(0x145c9, "Did the smugglers have anything to do with it?", 0x145f1);
        };
    };
    if (((BEN_POISON_SMUGGLERS) == (TRUE)) && ((BEN_POISON_HOWMANY) == (FALSE))) {
        INFO_ADDCHOICE(0x145c9, "How much did you get for kidnapping my brother?", 0x145f2);
    };
    if ((BEN_POISON_RENEGADES) == (FALSE)) {
        if ((LOG_GETSTATUS(MIS_Q205)) == (LOG_SUCCESS)) {
            INFO_ADDCHOICE(0x145c9, "Did the renegades have anything to do with it?", 0x145f3);
        };
    };
    if ((BEN_POISON_RING) == (FALSE)) {
        if ((NPC_HASITEMS(OTHER, 0x911a)) >= (1)) {
            INFO_ADDCHOICE(0x145c9, "You know that ring?", 0x145f4);
        };
    };
    if (((BEN_POISON_RING) == (TRUE)) && ((BEN_POISON_WHATRING) == (FALSE))) {
        INFO_ADDCHOICE(0x145c9, "Why would you recognize him by it?", 0x145f5);
    };
    if ((((BEN_POISON_RING) == (TRUE)) && ((BEN_POISON_WHATRING) == (TRUE))) && ((BEN_POISON_KILL) == (FALSE))) {
        INFO_ADDCHOICE(0x145c9, "I think I'm gonna hit you now. (Kill)", 0x145f6);
    };
    if ((BEN_POISON_KURT) == (FALSE)) {
        if (((GQ001_KURTFOUNDPOCKET01) == (TRUE)) || ((GQ001_KURTFOUNDPOCKET02) == (TRUE))) {
            INFO_ADDCHOICE(0x145c9, "Why didn't you take the gold from Kurt's hut?", 0x145f7);
        };
    };
    if ((BEN_POISON_SILBACH) == (FALSE)) {
        INFO_ADDCHOICE(0x145c9, "How did you get into the village?", 0x145f8);
    };
    if ((BEN_POISON_DICKON) == (FALSE)) {
        if ((GQ001_INFOFROMDICKON) == (TRUE)) {
            INFO_ADDCHOICE(0x145c9, "How did you get past Felix the sentry?", 0x145f9);
        };
    };
    if ((BEN_POISON_ESCAPE) == (FALSE)) {
        if (((BEN_POISON_SILBACH) == (TRUE)) || ((BEN_POISON_DICKON) == (TRUE))) {
            INFO_ADDCHOICE(0x145c9, "Why didn't you escape through one of the unguarded entrances?", 0x145fa);
        };
    };
    if ((Q307_BIGBENJORNQUESTIONS) >= (3)) {
        INFO_ADDCHOICE(0x145c9, "(FINISH CONVERSATION)", 0x145e3);
    };
}

var int BEN_Q307_DECISION = 0;
func void DIA_BEN_POISON_QUESTIONS_FINALMOVE() {
    INFO_CLEARCHOICES(0x145c9);
    if ((NPC_HASITEMS(OTHER, 0x9057)) >= (1)) {
        INFO_ADDCHOICE(0x145c9, "That's enough for me. Drink this.", 0x145e5);
    };
    INFO_ADDCHOICE(0x145c9, "That's enough for me. Now you can die in your ditch.", 0x145e6);
    INFO_ADDCHOICE(0x145c9, "(GET BACK TO THE QUESTIONS)", 0x145e4);
}
...
func void DIA_BEN_POISON_QUESTIONS_SAVEBEN() {
    ...
    B_GIVEINVITEMS(OTHER, SELF, 0x9057, 1);
    B_STANDUP();
    ...
    CREATEINVITEMS(SELF, 0x8cde, 1);
    AI_WAITTILLEND(SELF, OTHER);
    AI_STOPLOOKAT(SELF);
    AI_USEITEM(SELF, 0x8cde);
    AI_WAITTILLEND(SELF, OTHER);
...
func void DIA_BEN_POISON_INFO() {
    ...
    AI_FUNCTION(OTHER, 0x145cc);
    AI_OUTPUT(SELF, OTHER, "DIA_Ben_Poison_03_07");
    AI_OUTPUT(OTHER, SELF, "DIA_Ben_Poison_15_08");
    INFO_CLEARCHOICES(0x145c9);
    INFO_ADDCHOICE(0x145c9, "I have the antidote. I'll give it to you for information on Jorn.", 0x145eb);
    INFO_ADDCHOICE(0x145c9, "I hid the antidote well. Answer my questions.", 0x145ec);
}
...
func void DIA_BEN_POISON_QUESTIONS_WHO() {
    ...
    CREATEINVITEMS(SELF, 0x9d9e, 1);
    B_GIVEINVITEMS(SELF, OTHER, 0x9d9e, 1);
...
func int DIA_BEN_WHATHAPPEND_CONDITION() {
    if (((((KAPITEL) >= (5)) && ((Q307_POISONWAY) == (1))) && (NPC_KNOWSINFO(OTHER, 0x145fb))) && ((Q509_BENVOLFZACKE) == (0))) {
        return TRUE;
    };
    return 0 /* !broken stack! */;
}
...
func int DIA_BEN_Q509_QUESTION_CONDITION() {
    if ((((LOG_GETSTATUS(MIS_Q509)) == (LOG_RUNNING)) && ((Q509_COUNTWARRIORS) < (4))) && (NPC_KNOWSINFO(OTHER, 0x145fb))) {
        return TRUE;
    };
    return 0 /* !broken stack! */;
}
...
func void DIA_BEN_Q509_QUESTION_INFO() {
    ...
    AI_FUNCTION(SELF, 0x14605);
}

func void BEN_Q509_CHANGEARMOR() {
    CREATEINVITEMS(NONE_7500_BEN, 0x8a3b, 1);
    AI_EQUIPARMOR(NONE_7500_BEN, 0x8a3b);
...
func int DIA_BEN_Q509_FEEL_CONDITION() {
    if (((NPC_GETDISTTOWP(SELF, "PART5_Q509_BEN")) <= (0x7d0)) && ((Q509_BENVOLFZACKE) == (1))) {
        return TRUE;
    };
    return 0 /* !broken stack! */;
}
...
func int DIA_BEN_AMBIENT_CONDITION() {
    if ((NPC_KNOWSINFO(OTHER, 0x145b3)) && ((Q509_BENVOLFZACKE) != (1))) {
        return TRUE;
    };
    return 0 /* !broken stack! */;
}
...
func void DIA_BEN_PICKPOCKET_INFO() {
    INFO_CLEARCHOICES(0x1460c);
    INFO_ADDCHOICE(0x1460c, DIALOG_BACK, 0x14610);
    INFO_ADDCHOICE(0x1460c, DIALOG_PICKPOCKET, 0x1460f);
}

func void DIA_BEN_PICKPOCKET_DOIT() {
    if ((NPC_GETTALENTSKILL(OTHER, NPC_TALENT_PICKPOCKET)) >= (3)) {
        CREATEINVITEMS(SELF, 0x8e15, 2);
        B_GIVEINVITEMS(SELF, OTHER, 0x8e15, 2);

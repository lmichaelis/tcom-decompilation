...
func int SPELL_LOGIC_INFLATE(var int MANAINVESTED) {
    if (((NPC_GETACTIVESPELLISSCROLL(SELF)) && ((SELF.ATTRIBUTE[2]) >= (SPL_COST_SCROLL))) || ((SELF.ATTRIBUTE[2]) >= (SPL_COST_INFLATE))) {
        ...
        } else if (((((((((!(C_BODYSTATECONTAINS(OTHER, BS_SWIM))) && (!(C_BODYSTATECONTAINS(OTHER, BS_DIVE)))) && (!(C_NPCISDOWN(OTHER)))) && ((OTHER.GUILD) < (GIL_SEPERATOR_HUM))) && ((OTHER.FLAGS) != (NPC_FLAG_IMMORTAL))) && ((NPC_GETDISTTONPC(SELF, OTHER)) <= (1000))) && ((OTHER.GUILD) != (GIL_KDF))) && ((OTHER.GUILD) != (GIL_DMT))) && ((OTHER.GUILD) != (GIL_PAL))) {
            NPC_CLEARAIQUEUE(OTHER);
            B_CLEARPERCEPTIONS(OTHER);
            AI_STARTSTATE(OTHER, 0xab3a, 0, "");
        };
        return SPL_SENDCAST;
    };
    return SPL_SENDSTOP;
